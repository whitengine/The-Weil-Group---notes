...
Let $F$ be a \textit{complete} field with respect to a nonarquimidean absolute value $\abs \cdot _v$ (for example if $F$ is a local field). We will say that a polynomial $f \in \O_F [X]$ is \textbf {primitive}, if its reduction $\mod \primo p_F$ in $\kappa_F [X]$ is not the zero polynomial, i.e.
\[
